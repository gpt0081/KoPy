...
- PEFT에 특화된 긴 키워드 식별자도 pack 내부에서 음역합니다. 예: `lora_alpha → 로라_알파`, `target_modules → 타깃_모듈즈`, `task_type → 태스크_타입`, `lora_dropout → 로라_드롭아웃`.
- 과거 compact 표기(`겟페프트모델`, `아이에이쓰리컨피그`)는 기존 KoPy 코드 호환을 위한 입력 alias로 유지합니다.
- 새 학습 자료와 Python → KoPy 변환은 canonical 표기를 우선합니다.
- 한 글자인 `r`은 LoRA rank 외에도 일반 수학/반지름 변수로 널리 쓰여 모호하므로 현재는 원문을 유지합니다. 향후 함수 호출 키워드 문맥만 안전하게 식별하는 구조가 생기면 다시 감사합니다.
...
- LoRA/PEFT 키워드: `lora_alpha → 로라_알파`, `target_modules → 타깃_모듈즈`, `task_type → 태스크_타입`, `lora_dropout → 로라_드롭아웃`
...
`r=`은 매우 짧고 모호해서 현재 pack 활성화만으로 일반 식별자까지 바꾸는 것은 안전하지 않습니다. `bias=`, `inference_mode=`처럼 다른 라이브러리와 공유될 수 있는 키워드도 충돌 검증 없이 전역 단어표에 넣지 않습니다. 이들은 함수 호출 키워드 위치를 구분할 수 있는 context-aware 변환이 필요합니다.
