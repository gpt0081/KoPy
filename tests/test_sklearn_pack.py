...
class SklearnPackTests(unittest.TestCase):
    ...
    def test_from_import_and_estimator_methods_translate(self):
        source = (
            ...
            "엑스_트레인, 엑스_테스트, 와이_트레인, 와이_테스트 = 트레인테스트스플릿(엑스, 와이, test_size=0.25)\n"
            "모델 = 로지스틱리그레션()\n"
            "모델.핏(엑스_트레인, 와이_트레인)\n"
            "예측 = 모델.프리딕트(엑스_테스트)\n"
        )
        ...
        self.assertIn("X_train, X_test, y_train, y_test", python_source)
        self.assertIn("model.fit(X_train, y_train)", python_source)
        self.assertIn("model.predict(X_test)", python_source)
    ...
    def test_reverse_translation_keeps_submodule_path_stable(self):
        ...
        self.assertIn("스케일러 = 스탠더드스케일러()", kopy)
        self.assertIn(".핏트랜스폼(엑스)", kopy)
