...
class QdrantPackTests(unittest.TestCase):
    ...
    def test_qdrant_specific_client_methods_translate(self):
        ...
        self.assertIn("client.query_points(collection_name='docs', query=query, limit=3)", python_source)
    ...
    def test_unimported_qdrant_words_are_not_global(self):
        source = "client.쿼리포인츠(collection_name='docs', query=query)\n"
        self.assertEqual(translate(source).python, source)

    def test_python_to_kopy_preserves_domain_vocabulary(self):
        source = (
        ...
        for token in ("collection_name=", "vectors_config=", "query=", "limit="):
