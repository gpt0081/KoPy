...
class IrMeasuresPackTests(unittest.TestCase):
    ...
    def test_ir_measures_entry_points_translate(self):
        source = (
            ...
            "qrel = 큐렐('q1', 'd1', 1)\n"
            "doc = 스코어드독('q1', 'd1', 2.0)\n"
            "스코어즈 = 캘크어그리게이트([nDCG@10, P@5, RR], 큐렐즈, run)\n"
        )
        ...
        self.assertIn("scores = calc_aggregate([nDCG@10, P@5, RR], qrels, run)", python_source)

    def test_standard_metric_symbols_remain_python_data(self):
        source = (
            "프롬 아이알메저스 임포트 캘크어그리게이트, nDCG, P, RR\n"
            "metrics = 캘크어그리게이트([nDCG@10, P@5, RR], 큐렐즈, run)\n"
        )
        python_source = translate(source).python
    ...
    def test_unimported_words_are_not_global(self):
        source = "metrics = lib.캘크어그리게이트(measures, 큐렐즈, run)\n"
        translated = translate(source).python
        self.assertIn("lib.캘크어그리게이트", translated)
        self.assertIn("qrels", translated)

    def test_python_to_kopy_transliterates_common_ir_identifiers(self):
        source = (
        ...
        self.assertIn("메트릭 = 파스메저('nDCG@10')", kopy)
        self.assertIn("스코어즈 = 캘크어그리게이트([nDCG@10, P@5, RR], 큐렐즈, run)", kopy)
