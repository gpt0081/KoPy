...
class PyPdfPackTests(unittest.TestCase):
    ...
    def test_transferable_document_vocabulary_stays_python(self):
        source = (
            "프롬 파이피디에프 임포트 피디에프리더\n"
            "reader = 피디에프리더(path)\n"
            "pages = reader.pages\n"
    ...
    def test_python_to_kopy_round_trip(self):
        ...
        self.assertIn("reader = 피디에프리더(path)", kopy)
        self.assertIn("reader.pages[0].익스트랙트텍스트()", kopy)
        self.assertIn("writer.애드블랭크페이지(width=612, height=792)", kopy)
